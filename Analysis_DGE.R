...
twoGroupCompare <- function(Feature_Counts, Sample_Info, Gene_Info, Genes_Column_Name, Covariates = NULL, pvalue_Cutoff, log2Fold_Cutoff){
  ...
  # res_name = paste0(Variable_Of_Interest,"_", Groups_Selected[2], "_vs_", Groups_Selected[1])
  # res <- results(dds, name = res_name, alpha = 0.1)
  res <- results(dds, name = resultsNames(dds)[2], alpha=0.1)
